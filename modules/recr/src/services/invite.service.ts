...
import { recrInvitations, type RecrInvitation, type NewRecrInvitation } from '../entities/invite.schema';
...
export class InviteService {
  ...
  async sendInvitation(
    ...
    inviterId: string,
  ): Promise<{ invitation: RecrInvitation; rawToken: string }> {
    ...
    const existingInvitation = await this.db
      .select()
      .from(recrInvitations)
      .where(
        and(
          eq(recrInvitations.email, dto.email),
          eq(recrInvitations.role, dto.role),
          eq(recrInvitations.status, 'PENDING'),
        ),
      )
      .limit(1);
    ...
    const newInvitation: NewRecrInvitation = {
      email: dto.email,
      tokenHash,
      role: dto.role,
      inviterId,
  ...
  async verifyToken(
    token: string,
  ): Promise<{ invitation: RecrInvitation; valid: boolean; reason?: string }> {
    const tokenHash = this.hashToken(token);

    const [invitation] = await this.db
      .select()
      .from(recrInvitations)
      .where(eq(recrInvitations.tokenHash, tokenHash))
      .limit(1);
    ...
    if (new Date() > new Date(invitation.expiredAt)) {
      ...
      await this.db
        .update(recrInvitations)
        .set({
          status: 'EXPIRED',
          updatedAt: new Date(),
        })
        .where(eq(recrInvitations.id, invitation.id));
  ...
  async listInvitations(params: InvitationQueryParams): Promise<PaginatedInvitations> {
    ...
    if (employerId) {
      conditions.push(eq(recrInvitations.employerId, employerId));
    } else {
      // If no employer filter, only show invitations sent by this user
      conditions.push(eq(recrInvitations.inviterId, userId));
    }

    // Filter by status if provided
    if (status) {
      conditions.push(eq(recrInvitations.status, status));
    }
    ...
    const data = await this.db
      ...
      .from(recrInvitations)
      .where(whereClause)
      .orderBy(desc(recrInvitations.createdAt))
      .limit(limit)
      .offset(offset);
  ...
  async revokeInvitation(id: string, userId: string): Promise<RecrInvitation> {
    // 1. Find the invitation
    const [invitation] = await this.db
      .select()
      .from(recrInvitations)
      .where(eq(recrInvitations.id, id))
      .limit(1);
