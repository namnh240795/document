...
DIST_DIR="$PROJECT_ROOT/modules/dist"
...
# --- Copy CSS ---
cp "$TEMPLATES_DIR/style.css" "$DIST_DIR/style.css"
echo "  [OK] style.css"
...
for module in $MODULES; do
    ...
    MODULE_DOCS_DIR="$DIST_DIR/$MODULE_LOWER"
    MODULE_IMAGES_DIR="$MODULES_DIR/$MODULE_LOWER/images"

    mkdir -p "$MODULE_DOCS_DIR"

    # Copy module images to dist so they're accessible from the server
    if [ -d "$MODULE_IMAGES_DIR" ]; then
        cp -r "$MODULE_IMAGES_DIR" "$MODULE_DOCS_DIR/images"
    fi
    ...
    for template in module-srs.html module-tds.html module-database-design.html module-api-spec.html; do
        ...
        # Fix image paths for module docs
            ...
            while IFS= read -r line; do
                ...
                case "$src" in
                    ../images/${type}/*)
                        ...
                        if [ -n "$actual" ]; then
                            # Calculate relative path from dist/module/ to image
                            if [[ "$actual" == *"$MODULE_IMAGES_DIR"* ]]; then
                                rel_path="../$MODULE_LOWER/images/$type/$(basename "$actual")"
                            else
...
echo ""
echo "HTML documents ready in $DIST_DIR/"
echo ""

# Print first module doc
first_module=$(echo "$MODULES" | head -1)
if [ -n "$first_module" ]; then
    echo "Open: file://$(realpath "$DIST_DIR/index.html")"
fi
